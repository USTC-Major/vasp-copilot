...
VASP-Doctor 是面向 VASP 计算的源码应用。0.3.0 以 **Toolbox 独立基础计算**为主线：从已有结构准备输入，人工核对并交接到工作区，逐次批准远端写入和提交，查看监控与报告，并取回必要结果。AI 模式是可选客户端；不运行 AI 服务也能使用 Toolbox。解耦及 A—E 限定能力已进入当前代码，早期 D0 开发记录属于历史状态。
