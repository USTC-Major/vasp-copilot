...
固定模板或「固定 8 步锁死」。
本模块只保留最轻量分流：
- 流程已进入「待你确认提交」：这是真实提交入口（产品红线），交给 Orchestrator
  处理「确认提交 / 取消」；其余自由文本（补充输入/提建议/干预）交还 LLM 决策循环——绝不代替用户执行 sbatch。
...
def perform_submit(store: ProjectStore, project_id: str, task_id: str,
    ...
    """处理「确认提交」卡片：先落库授权，再按选择真实提交或取消。"""
    from .consent import get_card, resolve_card
    from .orchestrator import Orchestrator

    card = get_card(store, project_id, task_id, card_id)
    if card is None:
        return "确认卡片不存在或已处理，请重新发起确认。"
    resolve_card(store, project_id, task_id, card_id,
    ...
    if approved:
        ...
        return orch._submit(store, project_id, task_id, dict(flow))
...
def reply(store: ProjectStore, project_id: str, task_id: str, content: str,
    ...
    - 流程已在「待你确认提交」：交给 Orchestrator（真实提交入口）。
    ...
    if (flow.get("phase") == "await_submit"
        ...
        return _make_orchestrator(factory).handle(store, project_id, task_id,
                                                  content)
...
def reply_stream(store: ProjectStore, project_id: str, task_id: str,
    ...
    if (flow.get("phase") == "await_submit"
        ...
        if _is_cancel(content):
            answer = _make_orchestrator(factory).handle(store, project_id,
                                                        task_id, content)
