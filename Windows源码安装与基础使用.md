# Windows 源码安装与基础使用（0.3.0-rc.2 候选版）

以下命令面向新解压的源码目录、PowerShell 和本机浏览器。推荐本轮最终包验收环境的 Python 3.12.7、Node.js 24.19.0 与 npm，并需安装依赖所需网络；其他版本未在本轮干净安装中验证。使用普通用户权限即可。候选版尚未正式发布；仅将实际验收过的 Windows 源码 + ParaCloud/NICHE + Si2 单步 static 作为真实计算证据。
...
```powershell
$sourceRoot = 'C:\path\to\VASP-Doctor-0.3.0-rc.2'
...
```powershell
$sourceRoot = 'C:\path\to\VASP-Doctor-0.3.0-rc.2'
$dataRoot = Join-Path $env:LOCALAPPDATA 'VASP-Doctor\rc2-local'
